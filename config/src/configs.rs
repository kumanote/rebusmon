...
pub struct CheckerConfig {
    pub rebus_grpc_scheme: String,
    pub rebus_grpc_host: String,
    pub rebus_grpc_port: u16,
...
impl CheckerConfig {
    pub fn rebus_grpc_endpoint(&self) -> String {
        format!(
            "{}://{}:{}",
            self.rebus_grpc_scheme, self.rebus_grpc_host, self.rebus_grpc_port
        )
    }
}
...
impl SelfValidation for CheckerConfig {
    fn validate(&self) -> Result<()> {
        if self.rebus_grpc_scheme != "http" && self.rebus_grpc_scheme != "https" {
            return Err(anyhow!(
                "rebus daemon grpc_scheme must be either 'http' or 'https'..."
            ));
        }
...
impl FromEnv for CheckerConfig {
    fn from_env() -> Result<Self> {
        Ok(Self {
            rebus_grpc_scheme: "http".to_owned(),
            rebus_grpc_host: "127.0.0.1".to_owned(),
            rebus_grpc_port: 9090,
...
impl TryFrom<CheckerToml> for CheckerConfig {
    ...
    fn try_from(toml: CheckerToml) -> Result<Self> {
        ...
        if let Some(rebus_grpc_scheme) = toml.rebus_grpc_scheme {
            result.rebus_grpc_scheme = rebus_grpc_scheme;
        }
        if let Some(rebus_grpc_host) = toml.rebus_grpc_host {
            result.rebus_grpc_host = rebus_grpc_host;
        }
        if let Some(rebus_grpc_port) = toml.rebus_grpc_port {
            result.rebus_grpc_port = rebus_grpc_port;
        }
