...
FROM alpine:3.16
COPY --from=builder /rebusmon/target/release/rebusmon /usr/local/bin/rebusmon
RUN chmod +x /usr/local/bin/rebusmon
...
RUN addgroup -g ${GROUP_ID} rebusmon; \
    adduser -D -u ${USER_ID} -G rebusmon -h /rebusmon -s "/bin/bash" rebusmon;

# install su-exec
RUN apk add --no-cache su-exec; \
    su-exec rebusmon true;
