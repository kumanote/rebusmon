...
use anyhow::Context;
use logger::prelude::*;
use rebus_grpc_client as proto;
use rebus_grpc_client::tonic::{self, transport::Channel, Code};

#[derive(Debug, Clone)]
pub struct RebusdClient {
    endpoint: String,
...
impl RebusdClient {
    pub async fn establish_connection(endpoint: &str) -> Result<Channel> {
        tonic::transport::Endpoint::new(endpoint.to_owned())?
            .connect()
            .await
            .with_context(|| format!("failed to connect rebus daemon grpc endpoint: {}", endpoint))
    }
