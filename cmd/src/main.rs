...
#[structopt(about = "rebusmon running options")]
struct Opts {
...
fn main() {
    ...
    debug!(
        "Loaded rebus daemon monitoring tool config, config: {:?}",
        config
    );

    // set up rebus client pool
    let mut client_pool = HashMap::new();
    for checker in &config.checkers {
        let endpoint = checker.rebus_grpc_endpoint();
        info!("this tool will connect to {}", endpoint.as_str());
        let client = RebusdClient::new(endpoint.clone());
        client_pool.insert(endpoint, Arc::new(Mutex::new(client)));
    }
    rebuscli::set_client_pool(client_pool);

    if let Err(err) = rebusmon::start() {
        error!("{}", err);
    }
