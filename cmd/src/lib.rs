...
pub fn start() -> Result<()> {
    ...
    for checker in &app_config.checkers {
        ...
        let runtime = manager
            .rebusd_endpoint(checker.rebus_grpc_endpoint().as_str())
...
impl CheckManager {
    pub fn rebusd_endpoint(&mut self, rebusd_endpoint: &str) -> &mut Self {
        self.rebusd_endpoint = rebusd_endpoint.to_owned();
        self
    }
    ...
    fn setup(&mut self) -> tokio::runtime::Runtime {
        let runtime = tokio::runtime::Builder::new_multi_thread()
            .thread_name(format!("{}", self.rebusd_endpoint.as_str()))
            .enable_all()
            .build()
            .expect("Failed to create tokio runtime!");
        if self.check_if_syncing {
            ...
            let checker =
                checker::is_syncing::IsSyncingChecker::new(self.rebusd_endpoint.clone(), receiver);
    ...
    async fn next(&mut self) -> Result<&mut Self> {
        ...
        let latest_block_response = rebuscli::get_client(&self.rebusd_endpoint)
            .lock()
            .await
            .fetch_latest_block()
            .await?;
        ...
        if from_height <= latest_height {
            for height in from_height..=latest_height {
                let block_message: BlockMessage = if height == latest_height {
                    ...
                    let block_response = rebuscli::get_client(&self.rebusd_endpoint)
                        .lock()
                        .await
