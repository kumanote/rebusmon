...
use logger::prelude::*;
use rebus_grpc_client::{self as proto, prost};
...
impl NewProposalChecker {
    pub fn new(rebusd_endpoint: String, receiver: Receiver<NewProposalMessage>) -> Self {
        Self {
            rebusd_endpoint,
            receiver,
        }
                                                ...
                                                body.messages.iter().enumerate()
                                            {
                                                    ...
                                                    == "/cosmos.gov.v1beta1.MsgSubmitProposal"
                                                {
                                                    ...
                                                    let message_logs = if tx_message_logs.is_some()
                                                    {
                                                        ...
                                                        let tx_response = rebuscli::get_client(
                                                            &self.rebusd_endpoint,
                                                        )
                                                        .lock()
                                                        .await
